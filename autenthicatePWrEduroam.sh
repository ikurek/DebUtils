...
function confirm_exit {
  if [ ! -z $KDIALOG ] ; then
     if $KDIALOG --yesno "Naprawdę zakończyć?"  ; then
     exit 1
     fi
  fi
  if [ ! -z $ZENITY ] ; then
     if $ZENITY --question --text="Naprawdę zakończyć?" 2>/dev/null ; then
        exit 1
     fi
  fi
}
...
function user_cred {
  ...
  if ! USER_NAME=`prompt_nonempty_string 1 "wprowadź identyfikator użytkownika"` ; then
    exit 1
  fi

  while [ "$PASSWORD" != "$PASSWORD1" ]
  do
    if ! PASSWORD=`prompt_nonempty_string 0 "wprowadź hasło"` ; then
      exit 1
    fi
    if ! PASSWORD1=`prompt_nonempty_string 0 "powtórz hasło"` ; then
      exit 1
    fi
...
#prompt user for credentials
  ...
  if run_python_script ; then
   show_info "Instalacja zakończona"
else
   show_info "Nie udało się skonfigurować Network Managera, tworzę konfigurację wpa_suplicant"
   if ! ask "Network Manager configuration failed, but we may generate a wpa_supplicant configuration file if you wish. Be warned that your connection password will be saved in this file as clear text." "Zapisz plik" 1 ; then exit ; fi

if [ -f $HOME/.cat_installer/cat_installer.conf ] ; then
  if ! ask "Plik $HOME/.cat_installer/cat_installer.conf już istnieje i zostanie zastąpiony nowym." "Kontynuuj" 1 ; then confirm_exit; fi
  rm $HOME/.cat_installer/cat_installer.conf
  fi
   create_wpa_conf
   show_info "Wynik zapisano w $HOME/.cat_installer/cat_installer.conf"
fi
